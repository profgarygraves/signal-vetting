...
- **NOCCCD pre-screen** — the same intake fields and 8 screening questions NOCCCD Cybersecurity's TPRM process uses to decide whether a full assessment is even required (Department, Business Owner, New/Renewal, Contract Value, Go-Live Date, plus yes/no gates on data access/storage/processing, student/employee info, system integration, AI functionality, and confidential data). Kept verbatim so a completed pre-screen maps directly onto that intake form. The one addition: the AI question is binary in the source process, but AI risk isn't — answering "yes" triggers a structured follow-up (passive/embedded vs. generative vs. agentic) instead of one flat flag. An auto-computed Risk Tier (1/2/3, by confidential-data handling × blast radius) and "Assessment required?" result are shown live and included in every report.
